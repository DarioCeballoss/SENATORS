const app = new Vue({
  ...
  async function odena(mi) {
    ...
    ap.ordenadoM = await member.sort(function (pre, nex) {
      return pre.votes_with_party_pct - nex.votes_with_party_pct; //cambia prev nex mayor o menor
    });
  
    aps.ordenadoB = await member.sort(function (prev, next) {
      return next.votes_with_party_pct - prev.votes_with_party_pct;//cambia prev nex mayor o menor
    });
